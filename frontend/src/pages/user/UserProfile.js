...
function UserProfile(){
  ...
  const fetchUserData = async () => {
    ...
    try{
      ...
      const res = await axios.get(baseURL + '/api/users/user/details/', {headers:{
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    console.log(token);
    console.log("userupdate details",userUpdateDetails)
    let form_data = new FormData()
    form_data.append('profile_pic', userUpdateDetails.image, userUpdateDetails.image.name)
    let url = baseURL + '/api/users/user/details/update'
          ...
          <div className="card" style={{borderRadius:'15px'}}>
            <div className="card-body text-center">
              ...
              <br/>
                ...
                <br/>
                <button type="submit" className="btn btn-black btn-rounded btn-lg">
                  Update Profile Pic
                </button>
              </form>
              <br/>
